...
tr_group.add_argument('--seed', default=0, type=int,
                      help='random seed')
...
tr_group.add_argument('--num_checkpoints', dest='to_keep', default=2, type=int,
                      help='number of checkpoints to keep')
tr_group.add_argument('--cuda', default=0, type=str,
                      help='cuda NO.')
...
for dataset_name in datasets:
    ...
    with h5py.File(dataset_path, 'r') as f:
        for pdb_id in f:
            ...
            features[dataset_name].append(np.take(dataset, indices, axis=1))
...
# save mse
mse_t_all = pd.DataFrame(data={'Value': mse_t_all})
mse_v_all = pd.DataFrame(data={'Value': mse_v_all})

mse_t_all.to_csv(prefix + '-training_set_mse_all.csv', index=False)
mse_v_all.to_csv(prefix + '-validation_set_mse_all.csv', index=False)
